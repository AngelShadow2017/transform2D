...
namespace Core.TrueSync {
#if true
    /// <summary>
    /// Contains common Mathz operations.
    /// </summary>
    public sealed class TSMath {
        ...
        **/
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static FP LerpUnclamped(FP value1, FP value2, FP amount)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static FP FastQuadratic(FP x) {
            return FP.FastMul(x,x);
        }
        /// <summary>
        /// 用来更快地计算定点数的正整数幂
        /// </summary>
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static FP FastPow(FP a, int n)
        {
            FP ans = 1; // 赋值为乘法单位元，可能要根据构造函数修改
            while (n!=0)
            {
                if ((n & 1) != 0)
                {
        ...
        {
            FP num = Repeat(target - current, (FP)360);
            if (num > (FP)180)
            {
                num -= (FP)360;
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static FP DeltaRad(FP current, FP target)
        {
            FP num = Repeat(target - current, FP.PiTimes2);
            if (num > (FP)FP.Pi)
            {
                num -= (FP)FP.PiTimes2;
            }
...
public sealed class TSMath {
        ...
        **/
        ...
        /// Provides at least 6 decimals of accuracy.
        /// </summary>
        internal static FP Pow2(in FP x)
        {
            return Math.Pow(2,x._savedFloat);
        }
        ...
        /// The argument was non-positive
        /// </exception>
        const float log2_ = 0.6931472f;
        internal static FP Log2(in FP x)
        {
            return Math.Log(x._savedFloat) / log2_;
        }
        ...
        /// The argument was non-positive
        /// </exception>
        public static FP Ln(in FP x)
        {
            return Math.Log(x._savedFloat);
        }
        public static FP FastQuadratic(in FP x) {
            return FP.FastMul(x,x);
        }
        /// <summary>
        /// 用来更快地计算定点数的正整数幂
        ...
        public static FP FastPow(FP a, int n)
        {
            return Math.Pow(a._savedFloat,n);
        }
        /// <summary>
        /// Returns a specified number raised to the specified power.
        /// Provides about 5 digits of accuracy for the result.
        ...
        public static FP Pow(FP b, FP exp)
        {
            return Math.Pow(b._savedFloat, exp._savedFloat);
        }
