...
namespace Core.TrueSync
{
    ...
    public struct TSVector
    {
        ...
        public FP sqrMagnitude {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get {
                return FP.FromRaw(MathBurstedFix.DistanceSquared3D(this.x._serializedValue,this.y._serializedValue,this.z._serializedValue));
        ...
        public FP magnitude {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get {
                return FP.FromRaw(MathBurstedFix.Distance3D(this.x._serializedValue, this.y._serializedValue, this.z._serializedValue));
        ...
        public static TSVector MoveTowards(TSVector current, TSVector target, FP maxDistanceDelta)
        {
            ...
            FP toVector_x = target.x - current.x;
            FP toVector_y = target.y - current.y;
            FP toVector_z = target.z - current.z;

            FP sqdist = toVector_x * toVector_x + toVector_y * toVector_y + toVector_z * toVector_z;

            if (sqdist == 0 || (maxDistanceDelta >= 0 && sqdist <= maxDistanceDelta * maxDistanceDelta))
                return target;
            var dist = TSMath.Sqrt(sqdist);

            return new TSVector(current.x + toVector_x / dist * maxDistanceDelta,
        ...
        public override string ToString() {
            return string.Format("({0}, {1}, {2})", x.AsDouble(), y.AsDouble(), z.AsDouble());
        }
        #endregion
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static implicit operator TSVector2(TSVector ts) {
            return ts.ToTSVector2();
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
